...
then
	...
	echo 'String tomcat_web,port is 8089'
	
	t_pid=`/usr/sbin/lsof -n -P -t -i :8089`
	...
	mv /opt/huiliaoWar/huiliao_im_web.war /usr/local/tomcat_web/webapps/
...
then
	...
	echo 'String tomcat_service,port is 8090'
	
	t_pid=`/usr/sbin/lsof -n -P -t -i :8090`
	...
	mv /opt/huiliaoWar/huiliao_im_service.war /usr/local/tomcat_service/webapps/
